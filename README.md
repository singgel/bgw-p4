...
`bgw-p4` 是一套面向 Barefoot/Intel Tofino 可编程交换芯片的 BGW（Border Gateway）P4 数据面工程。仓库包含 P4-16/TNA 管线源码、BF-RT 表项初始化代码、交换机启动与运维脚本，以及基于 PTF 的回归测试框架。

## 功能概览

- 基于 P4-16 + TNA/T2NA 编写 BGW 转发管线。
- 支持 VXLAN 封装/解封装、VNI/隧道相关处理与外层 MAC 重写。
- 支持 ECMP/下一跳选择、哈希计算与多 pipeline 转发。
- 支持系统 ACL、镜像、meter、端口/VIF 流量统计等基础网关能力。
- 提供 BF-RT C/Python 表项封装与启动初始化脚本。
- 提供 PTF 测试目录，可用于数据面回归验证。

## 目录结构

> 当前源码主体位于仓库内的 `bgw-p4/` 子目录。
...
```bash
bgw-p4/p4src/bgw_sde_9.7_build.sh
```
...
```bash
bgw-p4/p4src/bgw_sde_9.3_build.sh
```
...
`bgw-p4/bgw-h/init_setup/` 包含设备初始化相关 C 代码与 switch API/BF-RT 封装，主要用于：
...
`bgw-p4/bgw-h/rpc_client/` 是 Python 初始化客户端，主入口为：

```bash
bgw-p4/bgw-h/rpc_client/bgw_start_up.py
```
...
启动逻辑包括 VXLAN rewrite、mirror、ECMP group、port stats、drop ACL 等表项初始化。运行前请确认：
...
`bgw-p4/tools/` 包含部署与运维相关工具，例如：

- `bfrt-apigen-tool/`：根据 BF-RT JSON 生成接口代码；
- `frr/`：FRR/BGP 配置示例；
- `ops/`：上线、恢复、监控、隔离等运维脚本；
...
- `hostif.json`、FRR 配置、端口映射、pipeline 数等均与部署环境强相关。
